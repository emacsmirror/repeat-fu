;;; repeat-fu-preset-bray.el --- Repeat for Bray modal editing -*- lexical-binding: t -*-
...
;;; Commentary:
;; Preset for Bray(experimental) modal editing.
;;
...
(declare-function bray-ex-state-insert "bray-ex" ())
(declare-function bray-ex-mark-set-on-motion-commands-no-repeat "bray-ex" ())
(declare-function bray-ex-mark-set-on-motion-commands-adjust "bray-ex" ())
(declare-function bray-ex-numeric-commands "bray-ex" ())
(declare-function bray-ex-mark-set-on-motion-commands "bray-ex" ())

(defconst repeat-fu-preset-bray--flag-is-active repeat-fu-flag-free)

(defun repeat-fu-preset-bray--pre-data ()
  "Pre command callback."
  (bray-state))

(defun repeat-fu-preset-bray--post-data (data-pre)
   ...
   (t
    (let* ((cmd-flag repeat-fu-this-command-bitmask)
      ...
      (when (and (region-active-p) (not deactivate-mark))
        (setq cmd-flag (logior cmd-flag repeat-fu-preset-bray--flag-is-active)))

      `[,cmd-flag ,this-command ,state-maybe-pair]))))

(defun repeat-fu-preset-bray--macros-extract (cmd-buffer)
  ...
  (let* ((vec (vconcat cmd-buffer))
         ...
         (elem-is-active-fn
          (lambda (i)
            (not
             (zerop (logand (aref (cdr (aref vec i)) 0) repeat-fu-preset-bray--flag-is-active)))))
      ...
      (cond
       ...
       (t
        ...
        (let ((index-min index-change)
              ...
              (state-insert (bray-ex-state-insert))
          ...
          (when do-scan-for-mark-commands
            ...
            (when (and (< index-max vec-end) (funcall elem-is-active-fn (1+ index-max)))
              (let ((index-max-orig index-max)
                  ...
                  (when (or (funcall elem-is-change-fn index-max)
                            (let ((cmd (funcall elem-cmd-fn index-max)))
                              (or
                               ...
                               (memq cmd (bray-ex-mark-set-on-motion-commands-no-repeat)))))
                    ;; Break.
                    (setq index-max vec-end)
                    (setq ok nil)))

                ;; Unlike "Meow", "Bray" only allows certain commands to activate the selection.
                ;; This could be relaxed as it's quite a constraint,
                ;; nevertheless, in Bray, selection is a deliberate operation,
                ...
                (let ((cmd (funcall elem-cmd-fn index-max)))
                  (unless (memq cmd (list 'bray-ex-reverse))
                    ...
                    (unless (memq
                             ...
                             (list 'bray-ex-region-toggle 'bray-ex-region-syntax-expand))
                      (setq ok nil)))
...
(defun repeat-fu-preset-bray--macros-select (kbuf-list)
  "Select the macro from KBUF-LIST."
  (nth
   (cond
    ((eq (bray-state) (bray-ex-state-insert))
...
;;;###autoload
(defun repeat-fu-preset-bray ()
  "Return the bray preset."
  (list
   :macros-extract #'repeat-fu-preset-bray--macros-extract
   :macros-select #'repeat-fu-preset-bray--macros-select
   :pre-data #'repeat-fu-preset-bray--pre-data
   :post-data #'repeat-fu-preset-bray--post-data))

(provide 'repeat-fu-preset-bray)
;; Local Variables:
;; fill-column: 99
;; indent-tabs-mode: nil
;; package-lint-main-file: "repeat-fu.el"
;; End:
;;; repeat-fu-preset-bray.el ends here
